...
import {
  actorProblems, credentialFindings, appendContribution, emptyBlock, foreignExecutionKey, SCHEMA_TAG,
...
/** The identity-bearing variables a spoke's ./ros reads (Praxis RQ-ROS-2026-A016). */
export const IDENTITY_VARIABLES = Object.freeze([
...
/**
 * The environment for a spoke's ./ros: the hub's environment with every
 * inherited identity variable removed, then the requester's declared identity
 * set explicitly. An unknown requester is declared as `unknown` so that a
 * spoke never attributes the request to the hub process or its runtime.
 * Unknown provider/model/runtime are omitted (the spoke records "unknown").
 */
export const spokeEnvironment = (hubEnv, requester) => {
  const scrubbed = Object.fromEntries(Object.entries(hubEnv).filter(([name]) => !IDENTITY_VARIABLES.includes(name)));
  const { actor, execution } = requester;
  return {
    ...scrubbed,
    ROS_ACTOR_KIND: actor.kind,
    ROS_ACTOR: actor.id,
    ...(actor.kind !== "human" && known(actor.provider) ? { ROS_TELEMETRY_PROVIDER: actor.provider } : {}),
    ...(actor.kind !== "human" && known(actor.model) ? { ROS_TELEMETRY_MODEL: actor.model } : {}),
    ...(actor.kind !== "human" && known(actor.runtime) ? { ROS_TELEMETRY_RUNTIME: actor.runtime } : {}),
    ...(execution !== undefined ? { ROS_EXECUTION_ID: execution } : {}),
  };
};
...
export const dispatchRecord = ({ operationId, at, repoId, command, hubActor, requester, legacyActor, spokeItemId }) => {
  const requesterKey = requester.execution ?? foreignExecutionKey("op", safeRunId(operationId));
  const hubKey = foreignExecutionKey("summa", safeRunId(operationId));
