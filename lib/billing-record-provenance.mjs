...
export const deriveBillingRecord = ({ billingRecordId, sources, creator, operationId, at }) => {
  const key = creator === undefined ? `EXT-summa.${escapeRunId(operationId)}`
  ...
  const lineage = [...new Set(sources.flatMap((source) => [...upstream(source), source.ref, ...(isNonEmptyString(source.workItemId) ? [source.workItemId] : [])]))];
  const record = {
